...
function wechat() {
  return {
    options: {},
    init_auth: function(companyId, url, callback) {
      ...
      if(!uid) {
        ...
        if(auth_info) {
          ...
          this.get_auth(companyId, url, callback);
        }
      } else {
    ...
    get_auth: function(companyId, url, callback) {
      ...
      this.post_data(API.gateway + "/wechat/web/get/jssdk", {
        ...
        if(data.code == 100000) {
          ...
          wx.config({
            ...
            jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', 'chooseImage', 'previewImage', 'uploadImage', 'downloadImage','hideAllNonBaseMenuItem']
          });
          call_back && call_back();
        } else {
